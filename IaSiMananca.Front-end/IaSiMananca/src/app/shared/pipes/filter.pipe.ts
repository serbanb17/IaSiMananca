  ...
  transform(arr: any[], filterObj: any): any {
    ...
    if (filterObj && filterObj.search) {
      ...
      if (filterObj.search.hasOwnProperty('name')) {
        ...
        for (i = 0; i < arr.length; i++) {
          var objectName = String(arr[i].name.toLowerCase());
          
            if (objectName.startsWith(name) || name === '') {
              filteredArr.push(arr[i]);
            }
    ...
    //   if (filterObj && filterObj.searchThroughDishes) {
    //     filteredArr = [];
    //     if (filterObj.search.hasOwnProperty('dishName')) {
    //       const dishName = filterObj.search['dishName'].toLowerCase();
    //       for (i = 0; i < arr.length; i++) {
    //         var objectName = String(arr[i].dishName.toLowerCase());
            
    //           if (objectName.startsWith(dishName) || dishName === '') {
    //             filteredArr.push(arr[i]);
    //           }
